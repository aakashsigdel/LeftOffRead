						...
						promisePanel.then(
							function onSuccess(myPanelfile) {
								...
								for(data in readJSON.allData)
									listData = listData +  '<LI><A id="'+ ((readJSON.allData.length - 1) - data) + '" href=#'+ ((readJSON.allData.length - 1) - data) + '>' + readJSON.allData[(readJSON.allData.length - 1) - data].title + '</A></LI>';
