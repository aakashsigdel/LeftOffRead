						...
						promisePanel.then(
							function onSuccess(myPanelfile) {
								...
								for(data in readJSON.allData)
									listData = listData +  '<div class="gradientBoxesWithOuterShadows"><A id="'+ ((readJSON.allData.length - 1) - data) + '" href=#'+ ((readJSON.allData.length - 1) - data) + '>' + readJSON.allData[(readJSON.allData.length - 1) - data].title + '</A></div>';

								myPanelfile.write(encoder.encode(listData));
								myPanelfile.write(encoder.encode('</body></html>'));
								myPanelfile.close();
							});
					});
			}
